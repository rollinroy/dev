...
#
local_dir=$local_root/$rsync_folder
if [ ! -d $local_dir ]; then
...
if [ $p_status -ne 0 ]; then
  ...
  echo $(date) : Copying $rsync_folder files from $rem_mac
fi
#
# mount imac
#
...
dst_mounted="no"
#
# copy imac PicturesAndMusic
#
rsync_folder="iMacDataSSD"
mnt_folder=$mnt_root"/imac_rsync_"$rsync_folder
mnt_remote="//"$rem_access"@"$rem_mac/$rsync_folder
mnt_info="//user:pwd@"$rem_mac/$rsync_folder
